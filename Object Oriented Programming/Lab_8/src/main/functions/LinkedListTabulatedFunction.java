...
public class LinkedListTabulatedFunction implements TabulatedFunctionImpl, Externalizable, Iterable<FunctionPoint> {
    ...
    public String toString() {
        ...
        builder.append("{").append(node.item);
        node = node.next;
        for (int i = 0; i < this.countOfPoints; ++i) {
            builder.append(", ").append(node.item.toString());
            node = node.next;
        }
        builder.append("}");
    ...
    public Iterator<FunctionPoint> iterator() {
        return new Iterator<>() {
            ...
            public FunctionPoint next() {
                ...
                FunctionPoint point = current.item;
     ...
     */
    public static class ListTabFuncFactory implements TabulatedFunctionFactory{

        @Override
        public TabulatedFunctionImpl createTabulatedFunction(double leftX, double rightX, int countOfPoints) {
            try {
                return new LinkedListTabulatedFunction(leftX, rightX, countOfPoints);
            } catch (InappropriateFunctionPointException e) {
                e.printStackTrace();
            }
            return null;
